...
def test_SAS():
    ...
    print(h2o.molecular_surface.setting)
...
def test_SAS_protein():
    ...
    prot.molecular_surface.setting["1"] = {"resolution": 0.4}
    ...
    prot.molecular_surface.setting.add("2", {"select": "A", "color": [0.8, 0.1, 0.1, 1.0]})
    prot.molecular_surface.draw()
...
def test_SES():
    ...
    h2o = molecule("H2O")
    h2o = Batoms("h2o", from_ase=h2o)
    h2o.molecular_surface.setting["1"].type = "SES"
    h2o.molecular_surface.draw()
...
def test_SES_protein():
    ...
    tstart = time()
    prot.molecular_surface.setting["1"].type = "SES"
    prot.molecular_surface.draw()
    t = time() - tstart
    assert t < 5
...
def test_molecular_surface_ops():
    ...
    assert len(c2h6so.molecular_surface.setting) == 2
    bpy.ops.surface.molecular_surface_remove(name="1")
    assert len(c2h6so.molecular_surface.setting) == 1
    print(c2h6so.molecular_surface.setting)
    bpy.ops.surface.molecular_surface_add(name="1")
    assert len(c2h6so.molecular_surface.setting) == 2
    c2h6so.molecular_surface.setting["1"].select = "1"
    c2h6so.molecular_surface.setting["2"].select = "2"
    c2h6so.molecular_surface.setting["2"].type = "SES"
