...
def get_cell(i1, i2):
    ...
    def getter(self):
        ...
        if context.object and context.object.batoms.type != "OTHER":
            obj = bpy.data.objects.get(f"{context.object.batoms.label}_cell")
            if obj is not None:
                return obj.data.vertices[i1 + 1].co[i2]
        return 0
...
def set_cell(i1, i2):
    ...
    def setter(self, value):
        ...
        if context.object and context.object.batoms.type != "OTHER":
            obj = bpy.data.objects.get(f"{context.object.batoms.label}_cell")
            if obj is not None:
                obj.data.vertices[i1 + 1].co[i2] = value
...
class CellProperties(bpy.types.PropertyGroup):
    ...
    pbc: BoolProperty(
        name="pbc", default=True, description="pbc", update=Callback_modify_pbc
    )
