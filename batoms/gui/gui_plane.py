...
from ..utils.butils import get_selected_batoms, get_selected_objects
...
class PlaneProperties(bpy.types.PropertyGroup):
    ...
    def Callback_modify_indices(self, context):
        ...
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "indices", plane.indices
        )

    def Callback_modify_distance(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "distance", plane.distance
        )

    def Callback_modify_scale(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "scale", plane.scale
        )

    def Callback_modify_crystal(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms,
            self.selected_plane,
            "crystal",
            plane.crystal,
            center=plane.center,
    ...
    def Callback_modify_center(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "center", plane.center
        )

    def Callback_modify_symmetry(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "symmetry", plane.symmetry
        )

    def Callback_modify_slicing(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "slicing", plane.slicing
        )

    def Callback_modify_boundary(self, context):
        plane = bpy.context.scene.batoms.plane
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "boundary", plane.boundary
        )
    ...
    def Callback_modify_show_edge(self, context):
        ...
        modify_plane_attr(
            self.selected_batoms, self.selected_plane, "show_edge", show_edge
        )
    ...
    indices: IntVectorProperty(
        ...
        update=Callback_modify_indices,
    )
    distance: FloatProperty(
        name="Distance",
        default=3,
        description="distance from origin",
        update=Callback_modify_distance,
    )
    scale: FloatProperty(
        name="Scale",
        default=1,
        description="scale of the plane",
        update=Callback_modify_scale,
    )
    ...
    show_edge: BoolProperty(
        ...
        update=Callback_modify_show_edge,
    )
