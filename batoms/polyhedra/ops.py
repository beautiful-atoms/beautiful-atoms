...
class PolyhedraAdd(OperatorBatoms):
    ...
    species: StringProperty(
        name="species", default="", description="species to be added"
    )
...
class PolyhedraRemove(OperatorBatoms):
    ...
    species: StringProperty(
        name="species", default="C", description="species to be removed"
    )

    all: BoolProperty(name="all", default=False, description="Remove all polyhedras")
    ...
    def execute(self, context):
        ...
        batoms.coll.Bpolyhedra.ui_list_index = min(
            max(0, index - 1), len(batoms.polyhedra.settings) - 1
        )
        context.view_layer.objects.active = obj
        return {"FINISHED"}
...
class PolyhedraModify(Operator):
    ...
    key: StringProperty(
        name="key", default="style", description="Replaced by this polyhedra"
    )
