...
def get_volumetric_data(self, context):
    keys = bpy.data.collections[
        context.object.batoms.label
    ].batoms.settings_volumetric_data.keys()
    items = []
    i = 0
    for key in keys:
...
class Bspecies(Base):
    ...
    elements: CollectionProperty(name="Belements", type=Belement)
    ...
    color: FloatVectorProperty(
        name="color", size=4, subtype="COLOR", min=0, max=1, default=(0, 0.2, 0.8, 1)
    )
...
class Batom(bpy.types.PropertyGroup):
    ...
    species: CollectionProperty(name="Bspecies", type=Bspecies)
...
class Battribute(Base):
    ...
    def __repr__(self) -> str:
        ...
        s = "{:20s}{:10s}{:10s}{:10s}{:10s}   {:20s}\n".format(
            "Name", "Type", "Domain", "delimiter", "Dimension", "Shape"
        )
...
class Bcell(bpy.types.PropertyGroup):
    ...
    color: FloatVectorProperty(
        name="color", size=4, subtype="COLOR", min=0, max=1, default=[0.2, 0.2, 0.2, 1]
    )
...
class Bboundary(Base):
    ...
    boundary: FloatVectorProperty(
        name="boundary", default=[0.0, 1.0, 0.0, 1.0, 0.0, 1.0], size=6
    )
    scale: FloatProperty(name="scale", soft_min=0.01, soft_max=1, default=0.01)
    color: FloatVectorProperty(
        name="color", size=4, subtype="COLOR", min=0, max=1, default=[0, 1, 1, 0.5]
    )
...
class Bselect(bpy.types.PropertyGroup):
    ...
    polyhedra_style: EnumProperty(
        ...
        items=(
            ("0", "atoms, bonds and polyhedra", "atoms, bonds and polyhedra"),
    ...
    bspecies: CollectionProperty(name="Bspecies", type=Bspecies)
...
class BatomsCollection(bpy.types.PropertyGroup):
    ...
    label: StringProperty(name="label", default="batoms")
    type: EnumProperty(
        name="type", description="Structural models", items=items, default="OTHER"
    )
    ...
    ui_list_index_species: IntProperty(name="ui_list_index_species", default=0)
    ui_list_index_select: IntProperty(name="ui_list_index_select", default=0)
    ui_list_index_volumetric_data: IntProperty(
        name="ui_list_index_volumetric_data", default=0
    )
    # collection
    settings_select: CollectionProperty(name="settings_select", type=Bselect)

    settings_species: CollectionProperty(name="settings_species", type=Bspecies)
    settings_volumetric_data: CollectionProperty(
        name="settings_volumetric_data", type=Bvolumetric_data
    )
...
class BatomsObject(bpy.types.PropertyGroup):
    ...
    # obj
    atom: PointerProperty(name="Batom", type=Batom)
    cell: PointerProperty(name="Bcell", type=Bcell)
    volume: PointerProperty(name="Bvolumetric_data", type=Bvolumetric_data)

    # collection
    settings_attribute: CollectionProperty(name="settings_attribute", type=Battribute)
