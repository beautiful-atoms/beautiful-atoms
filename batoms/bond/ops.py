...
class BondPairAdd(OperatorBatoms):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"}, "Add bond pair {} {}".format(self.species1, self.species2)
        )
        return {"FINISHED"}
...
class BondPairRemove(OperatorBatoms):
    ...
    name: StringProperty(
        name="name", default="C-H", description="Name of bond pair to be removed"
    )

    all: BoolProperty(name="all", default=False, description="Remove all bond pairs")
...
class BondModify(Operator):
    ...
    style: StringProperty(name="style", default="0", description="style")
