...
class IMPORT_OT_batoms(Operator, ImportHelper):
    ...
    label: StringProperty(name="label", default="", description="Name of batoms.")

    render: BoolProperty(
        name="Add a default render", default=True, description="Do you need a render?"
    )
...
def menu_func_import_batoms(self, context):
    lay = self.layout
    lay.operator(IMPORT_OT_batoms.bl_idname, text="Batoms file (xyz, cif, pdb, ...)")


def menu_func_export_batoms(self, context):
    lay = self.layout
    lay.operator(EXPORT_OT_batoms.bl_idname, text="Batoms file (xyz, cif, pdb, ...)")
