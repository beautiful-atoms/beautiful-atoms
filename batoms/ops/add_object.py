...
class AddMolecule(Operator):
    ...
    formula: StringProperty(name="Formula", default="CH4", description="formula")
...
class AddBulk(Operator):
    ...
    formula: StringProperty(name="Formula", default="Au", description="formula")

    crystalstructure: StringProperty(
        name="Crystalstructure", default="", description="Crystalstructure"
    )
    ...
    orthorhombic: BoolProperty(
        name="Orthorhombic", default=False, description="Orthorhombic"
    )

    cubic: BoolProperty(name="Cubic", default=False, description="Cubic")

    def execute(self, context):
        if self.label == "":
            self.label = self.formula
        a = None if self.latticeconstant[0] == 0 else self.latticeconstant[0]
        b = None if self.latticeconstant[1] == 0 else self.latticeconstant[1]
        c = None if self.latticeconstant[2] == 0 else self.latticeconstant[2]
        crystalstructure = (
            None if self.crystalstructure == "" else self.crystalstructure
        )
        atoms = bulk(
            self.formula,
...
class AddAtoms(Operator):
    ...
    formula: StringProperty(name="Formula", default="C", description="formula")
...
class AddSmiles(Operator):
    ...
    smiles: StringProperty(name="smiles", default="CCO", description="smiles")
